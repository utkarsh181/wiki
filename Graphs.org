...
Graph is defined as: G=(V,E), on set of vertices V, and contains a set of edges E.

* TYPES OF GRAPH

+ DIRECTED GRAPH - If edge have direction then graph is called directed graph.
...
+ SIMPLE GRAPH - Graph without self loop and parallel edges is called a simple graph.

+ NON DIRECTED GRAPH - Graph without directed edges are called non directed graph.
Non directed graph has no indegree and outdegree , but number of edges connecting a vertex is called degree.

+ NON CONNECTED GRAPH -  A graph in which there does not exist any path between at least one pair of vertices is called as a disconnected graph.

+ CONNECTED GRAPHS -  A graph in which there exixts at least one path between every pair of vertices are called connected graph
...
+ STRONGLY CONNECTED GRAPHS -  A graph in which from every vertex we can reach all other vertex in called strongly connected graphs.
...
+ DIRECTED ACYCLIC GRAPH - A graph not containing any cycle in it is called as an acyclic graph.
